...
A YouTube video that walks through this guide:
...
### Step 1b. Download this repository
...
We use this directory to test the sample model and our trained custom model that we look further in this aritcle
```
mv TensorFlow-Lite-Object-Detection-on-Android-and-Raspberry-Pi tflite1
cd tflite1
```
### MY IMAGE SHOULD BE REPLACED HERE
<p align="center">
...
First check whether the raspberry pi which you are using following the requirements or not. To check type the following commangs in the raspi.
...
```
```
uname -m ## this shows the architecture that the pi has.
```
```
sudo apt update
```
...
```
sudo apt install tensorflow --break-system-packages
```
You can check whether OpenCv and Tensor flow installed or not by opening the terminal and excuting the following.
...
```
```
pirnt(tf.__version__,cv2.__version__)
```
Another way:
From the download repository we can directly excute the command and download the requirements from the get_pi_requirements.
...
Here's the custom train Model that I used for detection.
The below link goes to the data set and how it executed.
